...
@section('content')
    <div class="console-title">
        <div class="float-right">
            <a href="{{admin_url('post/catlog/newcatlog')}}">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="dsxui-listtable">
                ...
                @foreach($catlogs as $catlog)
                    ...
                    <tr>
                        ...
                        <td width="60" class="align-center"><img src="{{image_url($catlog['icon'])}}" width="30" height="30" rel="pickimage" data-id="{{$catlog['catid']}}"></td>
                        ...
                        <td width="40">
                            <a href="{{admin_url('post/catlog/newcatlog?catid='.$catlog['catid'])}}">编辑</a>
                            <a href="{{admin_url('post/catlog/delete?catid='.$catlog['catid'])}}">删除</a>
                        </td>
                    </tr>
                    </tbody>
                    @foreach($catlog['childs'] as $child)
                        ...
                        <tr>
                            ...
                            <td width="60" class="align-center"><img src="{{image_url($child['icon'])}}" width="30" height="30" rel="pickimage" data-id="{{$child['catid']}}"></td>
                            <td>
                            ...
                            <td width="40">
                                <a href="{{admin_url('post/catlog/newcatlog?catid='.$child['catid'])}}">编辑</a>
                                <a href="{{admin_url('post/catlog/delete?catid='.$child['catid'])}}">删除</a>
                            </td>
                        </tr>
                        </tbody>
                        @foreach($child['childs'] as $sub)
                            ...
                            <tr>
                                ...
                                <td width="60" class="align-center">
                                    <img src="{{image_url($sub['icon'])}}" width="30" height="30" rel="pickimage" data-id="{{$sub['catid']}}">
                                </td>
                                <td>
                                    <div class="catlog-prefix">
                                        <div class="catlog-prefix">
                                            <input type="text" title="" class="form-control w200" name="catlogs[{{$sub['catid']}}][name]" value="{{$sub['name']}}" maxlength="10">
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td width="40">
                                    <a href="{{admin_url('post/catlog/newcatlog?catid='.$sub['catid'])}}">编辑</a>
                                    <a href="{{admin_url('post/catlog/delete?catid='.$sub['catid'])}}">删除</a>
                                </td>
                            </tr>
                            </tbody>
                        @endforeach
...
@section('foot')
    <script type="text/javascript">
        $("img[rel=pickimage]").on('click', function () {
            var img = $(this);
            var catid = img.attr('data-id');
            Widget.showImagePicker(function (data) {
                img.attr('src', data.image);
                $.post('{{admin_url('post/catlog/seticon')}}', {catid:catid,icon:data.image});
            });
        });
    </script>
@stop
