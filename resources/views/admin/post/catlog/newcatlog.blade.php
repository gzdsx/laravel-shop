...
@section('content')
    <div class="console-title">
        ...
        <h2>文章管理 > {{$catlog->catid ? '编辑分类' : '添加分类'}}</h2>
    </div>
        ...
        <form method="post" id="Form">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="dsxui-formtable">
                    ...
                    <td>
                        <select name="catlog[fid]" class="form-control custom-select w300" title="">
                            ...
                            @foreach ($catlogs as $cat)
                                <option value="{{$cat['catid']}}"{{$catlog->catid==$cat->catid ? ' selected="selected"' : ''}}>{{$cat['name']}}</option>
                                @foreach ($cat['childs'] as $child)
                                    <option value="{{$child['catid']}}"{{$catlog->catid==$child->catid ? ' selected="selected"' : ''}}>|--{{$child['name']}}</option>
                                @endforeach
                            @endforeach
                        </select>
                    </td>
                ...
                <tr>
                    <td>可发布内容</td>
                    <td>
                        <label><input type="radio" name="catlog[available]" value="1"{{$catlog->available ? ' checked="checked"' : ''}}> 是 </label>
                        <label><input type="radio" name="catlog[available]" value="0"{{!$catlog->available ? ' checked="checked"' : ''}}> 否</label>
                    </td>
