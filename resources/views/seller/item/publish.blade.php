...
@section('scripts')
    <script src="{{asset('kindeditor/kindeditor-all-min.js')}}" charset="utf-8"></script>
@stop
    ...
    <div class="console-title">
        <div class="float-right">
            <a href="{{url('seller/item')}}">
                <span class="btn btn-danger">商品列表</span>
            </a>
        </div>
        <h2>发布宝贝</h2>
    </div>
    <div class="content-div">
        <form method="post">
            ...
            <table cellspacing="0" cellpadding="0" width="100%" class="dsxui-formtable">
                ...
                <tr>
                    <td class="cell-name">商品类型</td>
                    <td>
                        <select class="form-control w300" name="item[type]">
                            @foreach (trans('item.item_types') as $k=>$v)
                                <option value="{{$k}}"{{$k==$item->type ? ' selected' : ''}}>{{$v}}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    <td class="cell-name"></td>
                    <td>
                        <div class="dsxui-uploader" id="itemImages">
                            <ul class="dsxui-uploader-files" id="dsxui-uploader-files">
                                <template v-for="(image, index) in images">
                                    <li class="dsxui-uploader-file bg-cover" :index="index" :style="{'background-image':'url('+image.thumb+')'}">
                                        <input type="hidden" :name="'images['+index+'][id]'" :value="image.id">
                                        <input type="hidden" :name="'images['+index+'][thumb]'" :value="image.thumb">
                                        <input type="hidden" :name="'images['+index+'][image]'" :value="image.image">
                                        <span class="iconfont icon-round_close_fill_light delete" @click="delImage(index)"></span>
                                    </li>
                                </template>
                            </ul>
                            <div class="dsxui-uploader-input" v-if="images.length < 5">
                                <div class="iconfont icon-add_light" @click="pickImage()"></div>
                            </div>
                        </div>
                    </td>
                </tr>
...
{{--                </tr>--}}
                ...
                <tr>
                    <td width="80"></td>
                    <td><button type="submit" class="btn btn-danger btn-lg w200" @click="submit()">立即发布</button></td>
                </tr>
                </tfoot>
            </table>
        </form>
...
@section('foot')
    <script type="text/javascript">var itemid='{{$item->itemid}}';</script>
    <script type="text/javascript">var images=@json($item->images);</script>
    <script src="{{asset('js/seller/pubitem.js?'.time())}}" type="text/javascript"></script>
@stop
