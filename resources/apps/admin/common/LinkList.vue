...
export default {
    ...
    methods: {
        fetchList() {
            let {cate_id} = this;
            this.$get('/links', {cate_id}).then(response => {
                this.dataList = response.result.items;
            });
        },
        fetchCategoryList() {
            this.$get('/link/categories').then(response => {
        ...
        onDelete() {
            ...
            this.$confirm('此操作将永久删除所选链接, 是否继续?', '提示', {
                ...
                this.$post('/link/delete', {ids}).then(response => {
                    this.fetchList();
                });
            });
        },
        ...
        onSubmit() {
            ...
            this.$post('/common/link.save', {link}).then(() => {
