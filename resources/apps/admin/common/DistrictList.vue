<template>
    <main-layout>
        <h2 slot="header">区位管理</h2>
        <div class="page-section">
            <header class="tablenav tablenav-top d-flex">
...
export default {
    ...
    methods: {
        fetchList() {
            let {parent_id} = this;
            this.loading = true;
            this.$get('/districts', {parent_id}).then(response => {
        ...
        fetchData() {
            let {parent_id} = this;
            if (parent_id) {
                this.$get('/district', {id: parent_id}).then(response => {
        ...
        onSubmit() {
            ...
            this.$post('/district', {district}).then(() => {
                this.fetchList();
                this.district = {};
        ...
        onDelete() {
            ...
            this.$confirm('此操作将永久删除所选信息, 是否继续?', '提示', {
                ...
                this.$post('/district/delete', {ids}).then(() => {
                    this.fetchList();
                });
            });
        },
