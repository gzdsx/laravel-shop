<template>
    <div>
        ...
        <image-picker :show.sync="showPicker" @confirm="handleConfirmImage"></image-picker>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            saveData(cb) {
                ...
                this.$post('/product/category/save', {
                    catid,
                    category
                }).then(response => {
                    this.resetData();
            ...
            handleDelete(catid) {
                this.$confirm('此操作将永久删除所选分类, 是否继续?', '提示', {
                    ...
                    this.$post('/product/category/delete', {catid}).then(response => {
                        this.fetchList();
                    });
                });
            },
            handleUpgrade(catid) {
                this.$post('/product/category/upgrade', {catid}).then(response => {
                    this.fetchList();
                });
            },
            handleIncrease(catid){
                this.$post('/product/category/increase', {catid}).then(response => {
                    this.fetchList();
                });
            },
            handleDecrease(catid) {
                this.$post('/product/category/decrease', {catid}).then(response => {
                    this.fetchList();
                });
            },
            handleUptop(catid) {
