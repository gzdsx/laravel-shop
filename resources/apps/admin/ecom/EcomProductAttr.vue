                ...
                <el-table :data="items" style="width: 100%" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column label="选项" width="170">
                        <template slot-scope="scope">
                            ...
                            <span>|</span>
                            <router-link :to="'/product/attrvalue?attr_cate_id='+scope.row.attr_cate_id">型号管理</router-link>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
        ...
        methods: {
            ...
            onDelete(attr_cate_id) {
                this.$confirm('此操作将永久删除所选分类, 是否继续?', '提示', {
                    ...
                    this.$post('/ecom/product/attr/delete', {attr_cate_id}).then(response => {
                        this.fetchList();
                    });
                });
            },
            onSubmit() {
                ...
                let attr_cate_id = attr.attr_cate_id;
                if (attr_cate_id) {
                    this.$post('/ecom/product/attr/update', {attr_cate_id, attr}).then(response => {
                        this.resetData();
                        this.fetchList();
                        this.showDialog = false;
                    });
                } else {
                    this.$post('/ecom/product/attr/create', {attr}).then(response => {
                        this.resetData();
                        this.fetchList();
                        this.showDialog = false;
                    });
