                ...
                <header class="table-edit-header">
                    <div class="display-flex">
                        ...
                        <div class="button-item">
                            <el-button type="primary" size="small" @click="handleShowAdd">添加型号</el-button>
                        </div>
                    </div>
                </header>
                <el-table :data="attr.attr_values">
                    <el-table-column label="型号名称" prop="attr_value"/>
...
<script>
    export default {
        ...
        methods: {
            fetchList() {
                let {attr_cate_id} = this;
                this.$get('/ecom/product/attr/info', {attr_cate_id}).then(response => {
                    this.attr = response.result.attr;
                });
            },
            onAdd() {
                this.resetData();
            ...
            onDelete(attr_id) {
                this.$confirm('此操作将永久删除所选分类, 是否继续?', '提示', {
                    ...
                    this.$post('/ecom/product/attrvalue/delete', {attr_id}).then(response => {
                        this.fetchList();
                    });
                });
            },
            onSubmit() {
                ...
                if (attr_id) {
                    this.$post('/ecom/product/attrvalue/update', {attr_id, attr_value}).then(response => {
                        this.resetData();
                        this.fetchList();
                        this.showDialog = false;
                    });
                } else {
                    this.$post('/ecom/product/attrvalue/create', {attr_value}).then(response => {
                        this.resetData();
                        this.fetchList();
                        this.showDialog = false;
                    });
