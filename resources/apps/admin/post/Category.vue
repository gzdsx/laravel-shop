                ...
                <tr>
                    ...
                    <td class="cell-input">
                        <el-select class="w-100" v-model="category.parent_id">
                            ...
                            <el-option
                                    v-for="(cate,idx) in dataList"
                                    :label="cate.cate_name"
                                    :value="cate.cate_id"
                                    :key="idx"
                                    v-if="cate.cate_id!==category.cate_id"
                            />
                        </el-select>
                    </td>
                    <td></td>
        ...
        methods: {
            ...
            onDelete(cate_id) {
                this.$confirm('此操作将永久删除所选分类, 是否继续?', '提示', {
                    ...
                    this.$post('/post/category.delete', {cate_id}).then(response => {
                        this.fetchList();
                    });
                });
            },
            onUpgrade(cate_id) {
                this.$post('/post/category.upgrade', {cate_id}).then(response => {
                    this.fetchList();
                });
            },
            onIncrease(cate_id) {
                this.$post('/post/category.increase', {cate_id}).then(response => {
                    this.fetchList();
                });
            },
            onDecrease(cate_id) {
                this.$post('/post/category.decrease', {cate_id}).then(response => {
                    this.fetchList();
                });
            },
            onShowAdd() {
