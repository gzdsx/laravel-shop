    ...
    <div class="container" style="background-color: #fff; padding-bottom: 50px;">
        <div class="cart">
            <van-checkbox-group v-model="result" @change="onChange">
                <van-swipe-cell :right-width="65" v-for="item in cart_DATA" :key="item.id">
                    <div class="cart-item">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onChange: function () {
                if (this.result.length === this.cart_DATA.length) {
                    this.checked = true;
                } else {
                    this.checked = false;
                }
            ...
            onDelete:function (itemid) {
                this.$axios.post('/h5/cart/delete',{items:[itemid]}).then(response=>{
                    this.cart_DATA = this.cart_DATA.filter(function (d) {
