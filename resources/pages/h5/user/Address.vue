...
<script>
    ...
    export default {
        ...
        methods: {
            fetchList: function () {
                this.$axios.get('/webapi/address/batchget').then(response => {
            ...
            handleSave: function (content) {
                ...
                this.$axios.post('/webapi/address/update', {address_id, address}).then(response => {
                    this.$toast.success({
                        message: '收货地址已保存',
                        onClose: () => {
                            this.addressInfo = {};
            ...
            handleDelete: function (addr) {
                this.showPopup = false;
                this.$axios.post('/webapi/address/delete', {address_id: this.addressInfo.id}).then(response => {
                    this.fetchList();
                });
            },
            ...
            handleEdit: function (addr) {
                this.$axios.get('/webapi/address/get?address_id=' + addr.id).then(response => {
