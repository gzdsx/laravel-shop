...
<script>
    ...
    export default {
        ...
        methods: {
            fetchList() {
                this.$get('/admin/district/batchget?fid=' + this.fid).then(response => {
                    this.itemList = response.data.items;
                });
            },
            getDistrict() {
                this.$get('/admin/district/get?id=' + this.fid).then(response => {
            ...
            handleSave() {
                ...
                this.$post('/admin/district/update', {
                    id: this.district.id,
            ...
            handleDelete() {
                ...
                this.$confirm('此操作将永久删除所选信息, 是否继续?', '提示', {
                    ...
                    this.$post('/admin/district/delete', {items}).then(response => {
                        this.fetchList();
                    });
                });
            },
            handleCheckAll(v){
                if (v) {
                    this.selectionIds = this.itemList.map((g)=>g.id);
                }else {
                    this.selectionIds = [];
                }
            }
