<template>
    <admin-frame>
        <header class="page-header">
            <div class="flex">
                <el-breadcrumb separator="/">
                    <el-breadcrumb-item>页面管理</el-breadcrumb-item>
                    <el-breadcrumb-item>编辑页面</el-breadcrumb-item>
                </el-breadcrumb>
            </div>
            <div>
                <router-link to="/pages/list">
                    <el-button type="primary" size="small">返回列表</el-button>
                </router-link>
            </div>
        </header>
        <div class="mainframe-content">
            <div class="content-block">
                <table class="dsxui-formtable">
                    <tr>
                        <td class="w60">标题</td>
                        <td>
                            <el-input size="medium" class="w300" v-model="pages.title"></el-input>
                        </td>
                        <td class="w60">别名</td>
                        <td>
                            <el-input size="medium" class="w300" v-model="pages.alias"></el-input>
                        </td>
                    </tr>
                    <tr>
                        <td>分类</td>
                        <td>
                            <el-select size="medium" class="w300" v-model="pages.catid">
                                <el-option
                                        v-for="(catlog,index) in catlogs"
                                        :value="catlog.pageid"
                                        :label="catlog.title"
                                        :key="index"
                                ></el-option>
                            </el-select>
                        </td>
                        <td>模板</td>
                        <td>
                            <el-input size="medium" class="w300" v-model="pages.template"></el-input>
                        </td>
                    </tr>
                </table>
                <table class="dsxui-formtable">
                    <tbody>
                    <tr>
                        <td class="w60">摘要</td>
                        <td>
                            <el-input type="textarea" rows="5" v-model="pages.summary"></el-input>
                        </td>
                    </tr>
                    <tr>
                        <td>内容</td>
                        <td>
                            <kind-editor v-model="pages.content"></kind-editor>
                        </td>
                    </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="h40"></div>
        <div class="edit-bottom">
            <el-button class="w100" @click="$router.go(-1)">取消</el-button>
            <el-button class="w100" type="primary" @click="submit">保存</el-button>
        </div>
    </admin-frame>
</template>

<script>
    import AdminFrame from "../common/AdminFrame";
    import KindEditor from "../../lib/KindEditor";

    export default {
        name: "PagesEdit",
        components: {
            AdminFrame,
            KindEditor
        },
        data: function () {
            return {
                pages: {
                    title: '',
                    catid: '',
                    alias: '',
                    template: '',
                    summary: '',
                    content: ''
                },
                pageid: '',
                catlogs: []
            }
        },
        mounted() {
            this.fetchCatlogs();
            var pageid = this.$route.params.pageid;
            if (pageid){
                this.$axios.get('/admin/pages/get?pageid='+pageid).then(response=>{
                    this.pages = response.data.pages;
                });
            }
        },
        methods: {
            fetchCatlogs: function () {
                this.$axios.get('/admin/pages/catlog/batchget').then(response => {
                    this.catlogs = response.data.items;
                });
            },
            submit: function () {
                if (!this.pages.title) {
                    this.$showToast('请填写页面标题');
                    return false;
                }

                if (!this.pages.catid) {
                    this.$showToast('请选择页面分类');
                    return false;
                }

                this.$axios.post('/admin/pages/update', {
                    pages: this.pages,
                    pageid: this.pageid
                }).then(response => {
                    this.$showToast('页面保存成功',()=>this.$router.go(0));
                });
            }
        }
    }
</script>

<style scoped>

</style>
