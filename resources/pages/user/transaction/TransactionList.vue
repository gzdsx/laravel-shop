        ...
        <section class="section">
            <table class="transaction-table">
                ...
                <tr v-for="(item,index) in itemList" :key="index">
                    ...
                    <td class="name">
                        <h3>{{item.subject}}</h3>
                        <p>流水:{{item.transaction_no}}</p>
                    </td>
                    <td class="amount">
                        <span style="color: #0BB20C;" v-if="item.payee_uid===user.uid">+{{item.amount}}</span>
                        <span style="color: #ff0000;" v-else>-{{item.amount}}</span>
                    </td>
                    <td class="align-right">{{item.transaction_state_des}}</td>
                </tr>
                </tbody>
            </table>
...
<script>
    export default {
        ...
        methods: {
            fetchList() {
                this.$get('/user/transaction/batchget', {
                    ...this.searchFields,
                    offset: this.offset
                }).then(response => {
