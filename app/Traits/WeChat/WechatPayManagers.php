...
trait WechatPayManagers
{
    ...
    public function unify(Request $request)
    {
        ...
        $appid = $request->input('appid') ?: $request->header('appid');
        $payment = $appid ? $this->payment($appid) : $this->payment();
        ...
        $unifiedOrder->setBody($transaction->subject)
            ...
            ->setOpenid($openid)
            ->setAttach($order->order_id)
            ->setTradeType($payment->config->get('trade_type', 'JSAPI'))
            ->setNotifyUrl('/notify/wechat/paid/' . $appid);
        ...
        if ($res->tradeSuccess()) {
            $prepay = PrePay::firstOrNew(['out_trade_no' => $unifiedOrder->getOutTradeNo()]);
            $prepay->uid = Auth::id();
    ...
    {
        return jsonError(500, $unifiedResponse->errCodeDes() ?: '', ['extra' => $unifiedResponse->all()]);
    }
}
